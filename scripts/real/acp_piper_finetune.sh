...
if [[ ! -f "${MIGRATED}" ]]; then
  echo "migrating LIBERO FastWAM to Piper 7D: ${MIGRATED}"
  "${PYTHON}" scripts/real/migrate_libero_fastwam_to_piper.py \
    --source "${LIBERO_FASTWAM}" \
    --output "${MIGRATED}"
fi

echo "Piper FastWAM finetune NUM_GPUS=${NUM_GPUS} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} MASTER_PORT=${MASTER_PORT}"
echo "PYTHON=${PYTHON} ACCELERATE=${ACCELERATE} output=${OUTPUT_DIR}"
piper_gpu_preflight
