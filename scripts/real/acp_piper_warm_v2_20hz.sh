...
# Uses every train window (2209 / epoch). Does not touch pilot_v1 or 16.67Hz.
...
TRAIN_WINDOWS="${TRAIN_WINDOWS:-2209}"
...
LOG="${WARM_DIR}/console.log"
echo "Piper 20Hz full-epoch FastWAM+WARM ACP start $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "${LOG}"
echo "processed=${PROCESSED}" | tee -a "${LOG}"
echo "NUM_GPUS=${NUM_GPUS} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}" | tee -a "${LOG}"
echo "fastwam_dir=${FASTWAM_DIR} warm_dir=${WARM_DIR}" | tee -a "${LOG}"
echo "fastwam_epochs=${FASTWAM_EPOCHS} warm_epochs=${WARM_EPOCHS} steps_per_epoch=${STEPS_PER_EPOCH} train_windows=${TRAIN_WINDOWS}" | tee -a "${LOG}"
echo "save_every=${SAVE_EVERY} eval_every=${EVAL_EVERY} eval_num_samples=0 (full DEV) zero=1" | tee -a "${LOG}"
...
echo "=== Stage A: full-epoch FastWAM on all 20Hz train windows ===" | tee -a "${LOG}"
launch \
...
if [[ -z "${FASTWAM_CKPT}" || ! -f "${FASTWAM_CKPT}" ]]; then
  echo "ERROR: Stage A FastWAM checkpoint missing under ${FASTWAM_DIR}/checkpoints/weights" | tee -a "${LOG}"
  exit 2
fi
echo "Stage A FastWAM checkpoint: ${FASTWAM_CKPT}" | tee -a "${LOG}"

echo "=== Stage B: full-epoch complete WARM on the 20Hz bank ===" | tee -a "${LOG}"
launch \
