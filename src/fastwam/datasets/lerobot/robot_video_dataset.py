...
class RobotVideoDataset(torch.utils.data.Dataset):
    ...
    def _get(self, idx):
        ...
        for attempt in range(self.max_padding_retry + 1):
            ...
            # Changing the requested sample index breaks the provenance-to-
            # candidate-cache contract used by WARM.  Strict mode keeps the
            # original padded sample and lets masks handle the boundary.
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.strict_sample_loading:
                raise RuntimeError(
                    f"Strict RobotVideoDataset loading failed for requested index {idx}"
                ) from e
