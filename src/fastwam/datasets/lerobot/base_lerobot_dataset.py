...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        episodes = {}
        if (episode_catalog_path is None) != (episode_split is None):
            raise ValueError(
                "episode_catalog_path and episode_split must be provided together"
            )
        if episode_catalog_path is not None:
            catalog = EpisodeCatalog.load(episode_catalog_path)
            ...
            if len(selected) != len(metas):
                raise ValueError(
                    "episode catalog dataset count does not match configured dataset_dirs"
                )
        ...
        while attempt < MAX_GETITEM_ATTEMPT:
            try:
                ...
                if self.strict_sample_loading:
                    raise RuntimeError(
                        f"Strict sample loading failed for requested index {idx}"
                    ) from err
                attempt += 1
                last_exception = err
                logger.warning(
